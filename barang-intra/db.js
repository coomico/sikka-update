// Constants
const DB_NAME = 'BarangIntraDB';
const DB_VERSION = 1;

// Store names
const STORES = {
  LAPORAN: 'laporan_intra',
  GROUP: 'group_intra',
  BARANG: 'barang_intra'
};
...
async function saveGroups(groups, laporanCreatedAt) {
  ...
  const promises = groups.map(group => {
    const groupData = {
      ...
      uraian: group.uraian,
      data_created_at: laporanCreatedAt
    };
...
async function getGroupsByLaporan(laporanCreatedAt) {
  ...
  try {
    ...
    if (store.indexNames.contains('data_created_at')) {
      const index = store.index('data_created_at');
      return new Promise((resolve, reject) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
          ...
          if (cursor) {
            ...
            if (group.data_created_at === laporanCreatedAt) {
              groups.push({
                ...group,
...
// Barang operations
async function saveBarang(barangList, kodeGroup, createdAt) {
  const db = await openDB();
  const tx = db.transaction(STORES.BARANG, 'readwrite');
  const store = tx.objectStore(STORES.BARANG);
  
  const groupId = `${kodeGroup}-${createdAt}`; // Create unique group identifier
...
async function getBarangByGroup(kodeGroup, createdAt) {
  const db = await openDB();
  const tx = db.transaction(STORES.BARANG, 'readonly');
  const store = tx.objectStore(STORES.BARANG);
  
  try {
    const groupId = `${kodeGroup}-${createdAt}`;
      ...
      try {
        ...
        if (cursor) {
          if (skipCount > 0) {
            ...
            // Get related groups - first check if index exists
            let groups = [];
            if (groupStore.indexNames.contains('data_created_at')) {
              const groupRequest = groupStore.index('data_created_at').getAll(laporanData.created_at);
              ...
              groups = await new Promise((resolveGroups, rejectGroups) => {
                ...
                groupRequest.onsuccess = (groupEvent) => {
                  ...
                  if (groupCursor) {
                    ...
                    if (group.data_created_at === laporanData.created_at) {
...
async function getLaporanById(createdAt) {
  ...
  try {
      ...
      // 2. Get groups associated with this laporan - first check if index exists
      const groupStore = tx.objectStore(STORES.GROUP);
      let groups = [];
      
      if (groupStore.indexNames.contains('data_created_at')) {
        const groupRequest = groupStore.index('data_created_at').getAll(laporanId);
        ...
        groups = await new Promise((resolve, reject) => {
          ...
          request.onsuccess = (event) => {
            ...
            if (cursor) {
              const group = cursor.value;
              if (group.data_created_at === laporanId) {
                foundGroups.push(group);
              }
              cursor.continue();
            } else {
...
async function deleteLaporan(createdAt) {
  ...
  try {
      ...
      if (groupStore.indexNames.contains('data_created_at')) {
        const request = groupStore.index('data_created_at').getAll(laporanId);
        groups = await new Promise((resolve, reject) => {
          request.onsuccess = () => resolve(request.result);
          request.onerror = () => reject(request.error);
        });
      ...
      for (const group of groups) {
          ...
          for (const barang of barangList) {
              await new Promise((resolve, reject) => {
                  ...
                  const deleteRequest = barangStore.delete([barang.kode, barang.group_id, barang.data_created_at]);
          ...
          await new Promise((resolve, reject) => {
              // Need to use the composite key
              const deleteRequest = groupStore.delete([group.kode, group.data_created_at]);
              deleteRequest.onsuccess = () => resolve();
              deleteRequest.onerror = () => reject(deleteRequest.error);
          });
      }
